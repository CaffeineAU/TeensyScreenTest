...
////===========================================================
//// Try Draw using writeRect
//bool bmpDraw(const char *filename, uint8_t x, uint16_t y, int scale) {
//
//	File     bmpFile;
//	int      bmpWidth, bmpHeight;   // W+H in pixels
//	uint8_t  bmpDepth;              // Bit depth (currently must be 24)
...
//					}
//
//					for (int i = ROWSPERDRAW; i > 0; i -= scale)
//					{
//						int index = 0;
//						for (col = 0; col < (bmpWidth); col += scale) { // For each pixel...
//																		// Time to read more pixel data?
//							if (buffidx >= sizeof(sdbuffer)) { // Indeed
//								bmpFile.read(sdbuffer, sizeof(sdbuffer));
//								buffidx = 0; // Set index to beginning
...
//							}
//
//							awColors[(index++) + (((i / scale) - 1)*w)] = tft.color565(r, g, b);
//							Serial.print("col : ");
//							Serial.print(col);
...
//							Serial.println((index)+(((i / scale) - 1)*w));
//						} // end pixel
//					}
//					tft.writeRect(0, row / scale, w, ROWSPERDRAW / scale, awColors);
//				} // end scanline
//				Serial.print(F("Loaded in "));
//				Serial.print(millis() - startTime);
//				Serial.println(" ms");
